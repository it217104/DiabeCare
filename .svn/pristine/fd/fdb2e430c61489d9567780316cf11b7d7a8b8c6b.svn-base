...
public partial class FoodPage : ContentPage
{
    ...
    private async Task GetNutrients()
    {
        try
        {
            ...
            {
                mFood = response.foods.FirstOrDefault();
                lblCalories.Text = $"{mFood.nf_calories} Kcal: {mFood.nf_protein}gr Protein, {mFood.nf_total_carbohydrate}gr Carbohydrate, {mFood.nf_total_fat}gr Fat, {mFood.nf_sugars} Sugar";
                if (pkFood.SelectedIndex == 0 && mFood.nf_total_carbohydrate > 30)
                {
                    await App.Current.MainPage.DisplayAlert("Alert!", "Carbohydrate Intake for this meal is higher than recommended!", "OK");
                }
                if (pkFood.SelectedIndex == 1 && mFood.nf_total_carbohydrate > 60)
                {
                    await App.Current.MainPage.DisplayAlert("Alert!", "Carbohydrate Intake for this meal is higher than recommended!", "OK");
                }
            }
        ...
        try
        {
            ...
            else
            {
                FoodData foodData = new()
                {
                    ...
                    FoodCalories = mFood.nf_calories
                };
